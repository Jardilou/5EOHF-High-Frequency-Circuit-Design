...
def main():
    ...
    L1 = 2.0   # [mm] initial vertical section from feed
    L2 = 22.5  # [mm] shorting stub length
    L3 = 3.0   # [mm] horizontal meander sections
    L4 = 45.0  # [mm] total vertical extent of meander
    ...
    pcb_margin = (2.5, 6, 2.5, 63)  # [mm] margin between traces and PCB edge (left, top, right, bottom)
    margin = 100 * mm  # [mm] margin around PCB for air box  (30)

    # --- 3. Create simulation object --------------------------------------------------------------------------------------------------------------------------
    model = em.Simulation("IFA", loglevel="INFO")

    # --- 4. Define PCB MIFA antenna geometry -----------------------------------------------------------------------------------------------------------------------
    pcbl = em.geo.PCB(thickness=th, unit=mm, material=em.lib.AIR)
    ...
    ant = (
        ...
        .straight(L2)  # First horizontal section (L3) - right
        .turn(-90, corner_type="square")  # Turn up
        .straight(L3)  # Vertical segment
        .turn(-90, corner_type="square")  # Turn left
        .straight(L4)  # Second horizontal section (L3) - left
        .turn(90, corner_type="square")  # Turn up
        .straight(L3)  # Vertical segment
        .turn(90, corner_type="square")  # Turn right
        .straight(L4)  # Third horizontal section (L3) - right
        .turn(-90, corner_type="square")  # Turn up
        .straight(L3)  # Vertical segment
        .turn(-90, corner_type="square")  # Turn left
    ...
    ground_bottom = em.geo.XYPlate(
        ...
        -63 * mm,
        (x0 * mm - Y * mm / 2, y0 * mm, pcbl.z(1) * mm),
    ...
    print("PCBL Length : ", pcbl.length)
    print("PCBL Width : ", pcbl.width)
    ...
    model.mw.set_resolution(0.5)   # 0.4 ou même 0.5 mm au début  (0.2)
    # Frequency sweep across the resonance
    model.mw.set_frequency_range(f1, f2, 25)

    # --- Combine geometry into simulation 
    model.commit_geometry()
    model.mesher.set_boundary_size(ant_trace, 2 * mm, growth_rate=5)
    model.mesher.set_boundary_size(air, 30 * mm)
    ...
    surf = ff3d.surfplot(
        ...
        dB=False,
        rmax=40 * mm,
        offset=(0, (32)/2 * mm, 0),  # type: ignore
    )
